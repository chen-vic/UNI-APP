					...
					<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback">
						<view class="card_item flex flex-row" v-for="(item, index) in dataList" :key="index" @click="toDetails(item.service_id)">
							...
							<view class="card_item_con">
								...
								<view class="card_info"><text>预约时间：{{item.order_date}}</text></view>
								<view class="card_info">{{item.member_name}},{{item.mobile_phone}}</view>
							</view>
						</view>
					</mescroll-body>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				...
				apiMyServices(page.num, page.size, member_id).then(curPageData => {
