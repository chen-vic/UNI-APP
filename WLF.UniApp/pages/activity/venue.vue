					...
					<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback">
						<view class="card_item flex flex-row" v-for="(item, index) in dataList" :key="index" @click="toDetails(item.event_id)">
							...
							<view class="card_item_con">
								<view class="">
									<text class="mini_tag" v-if="!item.is_free">免费</text>
								</view>
								<view class="card_title">{{item.event_title}}</view>
								<view class="card_info" style="overflow: hidden;">
									<image src="/static/me/ic_adress@2x.png" class="icon24"></image>{{item.event_address}}
								</view>
								<!-- <view class="card_info">{{item.event_address}}</view> -->
								<view class="card_info">浏览{{item.visit_num}}</view>
							</view>
						</view>
					</mescroll-body>
...
<script>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				//联网加载数据
				apiEventsVenue(page.num, page.size, this.service_type).then(curPageData => {
			...
			toDetails(id) {
				uni.navigateTo({
					url: `/pages/activity/venuedetails?id=${id}`
				})
			}
		}
	}
